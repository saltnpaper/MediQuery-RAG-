...
Table~\ref{tab:retrievalcompare} summarizes retrieval performance for pre-trained versus
fine-tuned retrieval components. The fine-tuned setup improves every reported metric,
both before and after reranking. The gains are largest for reranked MRR and reranked
NDCG@5, suggesting that fine-tuning improves not only whether the correct chunk is
retrieved, but also how high it is ranked among the final candidates shown to the
generator.

\begin{table}[h]
\centering
\caption{Retrieval Comparison: Pre-trained vs.\ Fine-tuned}
\label{tab:retrievalcompare}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
...
\noindent
These results show that the fine-tuned retriever and reranker are better aligned with Medicare policy search than the pre-trained models. In particular, the 0.1380 gain in reranked Recall@5 means the final top-5 context is more likely to include the correct evidence.
